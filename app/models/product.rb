class Product < ActiveRecord::Base
  ...
  def self.sold_in_with_quantity(start_time, end_time)
    joins(:tab_items)
      .where('tab_items.created_at >  ? AND tab_items.created_at < ? AND tab_id IS NOT NULL', start_time, end_time)
      .select("products.id, plu, title, product_group, product_group_id, product_type, SUM(quantity) AS sum_quantity, (price * SUM(quantity)) AS sum_price")
